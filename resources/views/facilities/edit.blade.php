@extends('layouts.app')

@section('title', 'Ubah Fasilitas: ' . $facility->code)
@section('page_title', 'Ubah Fasilitas: ' . $facility->code)
@section('page_subtitle', 'Perbarui rincian inventaris, kondisi fisik, atau catatan fasilitas kos.')

@section('content')
<div class="row justify-content-center">
    <div class="col-12 col-lg-8">
        <div class="card simkos-card">
            <div class="card-header bg-white py-3 d-flex align-items-center justify-content-between">
                <span class="h6 fw-bold mb-0 text-dark">Formulir Pembaruan Fasilitas</span>
                <span class="badge {{ $facility->isArchived() ? 'bg-secondary' : 'bg-success' }}">
                    {{ $facility->isArchived() ? 'Status: Diarsipkan' : 'Status: Aktif' }}
                </span>
            </div>
            <div class="card-body p-4">
                @if($hasComplaints)
                    <div class="alert alert-info py-2 px-3 small mb-3 border-info">
                        <strong>Perhatian Integritas Riwayat Keluhan:</strong><br>
                        Fasilitas ini telah memiliki riwayat keluhan. Sesuai kebijakan bisnis kos, lokasi penempatan dikunci dan tidak dapat diubah agar penelusuran histori tetap akurat. Jika fasilitas dipindahkan ke lokasi lain, silakan arsipkan fasilitas ini dan daftarkan fasilitas baru.
                    </div>
                @endif

                @if($facility->location_type === 'room' && $facility->room?->isArchived())
                    <div class="alert alert-warning py-2 px-3 small mb-3 border-warning">
                        <strong>Kamar Sedang Diarsipkan:</strong><br>
                        Kamar <strong>{{ $facility->room->number }}</strong> saat ini berstatus diarsipkan. Anda diperbolehkan memperbarui informasi nama, kondisi, dan catatan fasilitas ini tanpa harus memindahkan kamar. Jika ingin memindahkan fasilitas ke kamar lain, pilih kamar yang aktif.
                    </div>
                @endif

                <form method="POST" action="{{ route('facilities.update', $facility) }}" novalidate id="facilityEditForm">
                    @csrf
                    @method('PUT')

                    {{-- Kode Fasilitas --}}
                    <div class="mb-3">
                        <label for="code" class="form-label">
                            Kode Fasilitas <span class="text-danger" aria-hidden="true">*</span>
                        </label>
                        <input
                            type="text"
                            name="code"
                            id="code"
                            value="{{ old('code', $facility->code) }}"
                            class="form-control font-monospace @error('code') is-invalid @enderror"
                            required
                            maxlength="30"
                            aria-describedby="code-help @error('code') code-error @enderror"
                            @error('code') aria-invalid="true" @enderror
                        >
                        <div id="code-help" class="form-text">
                            Kode unik fasilitas (maksimal 30 karakter).
                        </div>
                        @error('code')
                            <div id="code-error" class="invalid-feedback d-block" role="alert">
                                {{ $message }}
                            </div>
                        @enderror
                    </div>

                    {{-- Nama Fasilitas --}}
                    <div class="mb-3">
                        <label for="name" class="form-label">
                            Nama Fasilitas <span class="text-danger" aria-hidden="true">*</span>
                        </label>
                        <input
                            type="text"
                            name="name"
                            id="name"
                            value="{{ old('name', $facility->name) }}"
                            class="form-control @error('name') is-invalid @enderror"
                            required
                            minlength="2"
                            maxlength="100"
                            aria-describedby="name-help @error('name') name-error @enderror"
                            @error('name') aria-invalid="true" @enderror
                        >
                        <div id="name-help" class="form-text">
                            Nama deskriptif barang fasilitas (2 sampai 100 karakter).
                        </div>
                        @error('name')
                            <div id="name-error" class="invalid-feedback d-block" role="alert">
                                {{ $message }}
                            </div>
                        @enderror
                    </div>

                    {{-- Kondisi Fasilitas --}}
                    <div class="mb-3">
                        <label for="condition" class="form-label">
                            Kondisi Fasilitas <span class="text-danger" aria-hidden="true">*</span>
                        </label>
                        <select
                            name="condition"
                            id="condition"
                            class="form-select @error('condition') is-invalid @enderror"
                            required
                            aria-describedby="condition-help @error('condition') condition-error @enderror"
                            @error('condition') aria-invalid="true" @enderror
                        >
                            <option value="good" {{ old('condition', $facility->condition) === 'good' ? 'selected' : '' }}>Baik (Berfungsi Normal)</option>
                            <option value="broken" {{ old('condition', $facility->condition) === 'broken' ? 'selected' : '' }}>Rusak (Perlu Tindakan)</option>
                            <option value="repairing" {{ old('condition', $facility->condition) === 'repairing' ? 'selected' : '' }}>Dalam Perbaikan (Sedang Dikerjakan)</option>
                        </select>
                        <div id="condition-help" class="form-text">
                            Status kelayakan fisik inventaris fasilitas saat ini.
                        </div>
                        @error('condition')
                            <div id="condition-error" class="invalid-feedback d-block" role="alert">
                                {{ $message }}
                            </div>
                        @enderror
                    </div>

                    {{-- Tipe Lokasi Penempatan --}}
                    <div class="mb-3">
                        <label class="form-label d-block">
                            Tipe Lokasi Penempatan
                            @if($hasComplaints)
                                <span class="badge bg-secondary-subtle text-secondary border ms-1">Terkunci (Ada Keluhan)</span>
                            @else
                                <span class="text-danger" aria-hidden="true">*</span>
                            @endif
                        </label>
                        <div class="form-check form-check-inline">
                            <input
                                class="form-check-input @error('location_type') is-invalid @enderror"
                                type="radio"
                                name="location_type"
                                id="location_type_room"
                                value="room"
                                {{ old('location_type', $facility->location_type) === 'room' ? 'checked' : '' }}
                                {{ $hasComplaints ? 'disabled' : '' }}
                                onchange="toggleLocationInputs()"
                            >
                            <label class="form-check-label fw-semibold text-dark" for="location_type_room">
                                Kamar Kos
                            </label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input
                                class="form-check-input @error('location_type') is-invalid @enderror"
                                type="radio"
                                name="location_type"
                                id="location_type_shared"
                                value="shared"
                                {{ old('location_type', $facility->location_type) === 'shared' ? 'checked' : '' }}
                                {{ $hasComplaints ? 'disabled' : '' }}
                                onchange="toggleLocationInputs()"
                            >
                            <label class="form-check-label fw-semibold text-dark" for="location_type_shared">
                                Area Bersama (Fasilitas Umum)
                            </label>
                        </div>
                        @error('location_type')
                            <div class="invalid-feedback d-block" role="alert">
                                {{ $message }}
                            </div>
                        @enderror
                    </div>

                    {{-- Pilihan Kamar (Tampil jika tipe = room) --}}
                    <div class="mb-3" id="container_room_id">
                        <label for="room_id" class="form-label">
                            Kamar Penempatan
                            @if($hasComplaints)
                                <span class="badge bg-secondary-subtle text-secondary border ms-1">Terkunci</span>
                            @else
                                <span class="text-danger" aria-hidden="true">*</span>
                            @endif
                        </label>
                        <select
                            name="room_id"
                            id="room_id"
                            class="form-select @error('room_id') is-invalid @enderror"
                            {{ $hasComplaints ? 'disabled' : '' }}
                            aria-describedby="room_id-help @error('room_id') room_id-error @enderror"
                            @error('room_id') aria-invalid="true" @enderror
                        >
                            <option value="">-- Pilih Kamar --</option>
                            @foreach($rooms as $r)
                                <option
                                    value="{{ $r->id }}"
                                    {{ old('room_id', $facility->room_id) == $r->id ? 'selected' : '' }}
                                >
                                    Kamar {{ $r->number }} ({{ $r->type }}){{ $r->isArchived() ? ' - [Kamar Diarsipkan]' : '' }}
                                </option>
                            @endforeach
                        </select>
                        <div id="room_id-help" class="form-text">
                            @if($hasComplaints)
                                Lokasi kamar tidak dapat diubah karena fasilitas memiliki riwayat keluhan.
                            @else
                                Kamar tujuan perpindahan harus berupa kamar yang aktif.
                            @endif
                        </div>
                        @error('room_id')
                            <div id="room_id-error" class="invalid-feedback d-block" role="alert">
                                {{ $message }}
                            </div>
                        @enderror
                    </div>

                    {{-- Input Nama Area Bersama (Tampil jika tipe = shared) --}}
                    <div class="mb-3 d-none" id="container_area_name">
                        <label for="area_name" class="form-label">
                            Nama Area Bersama
                            @if($hasComplaints)
                                <span class="badge bg-secondary-subtle text-secondary border ms-1">Terkunci</span>
                            @else
                                <span class="text-danger" aria-hidden="true">*</span>
                            @endif
                        </label>
                        <input
                            type="text"
                            name="area_name"
                            id="area_name"
                            value="{{ old('area_name', $facility->area_name) }}"
                            class="form-control @error('area_name') is-invalid @enderror"
                            placeholder="Contoh: Dapur Lantai 1, Ruang Tamu, Garasi"
                            maxlength="100"
                            {{ $hasComplaints ? 'disabled' : '' }}
                            aria-describedby="area_name-help @error('area_name') area_name-error @enderror"
                            @error('area_name') aria-invalid="true" @enderror
                        >
                        <div id="area_name-help" class="form-text">
                            @if($hasComplaints)
                                Lokasi area bersama tidak dapat diubah karena fasilitas memiliki riwayat keluhan.
                            @else
                                Nama lokasi area bersama fasilitas umum (2 sampai 100 karakter).
                            @endif
                        </div>
                        @error('area_name')
                            <div id="area_name-error" class="invalid-feedback d-block" role="alert">
                                {{ $message }}
                            </div>
                        @enderror
                    </div>

                    {{-- Catatan (Textarea) --}}
                    <div class="mb-3">
                        <label for="notes" class="form-label">
                            Catatan / Keterangan
                            <span class="text-muted fw-normal small">(Opsional)</span>
                        </label>
                        <textarea
                            name="notes"
                            id="notes"
                            rows="3"
                            class="form-control @error('notes') is-invalid @enderror"
                            placeholder="Keterangan nomor seri, tanggal pembelian, atau garansi..."
                            maxlength="1000"
                            aria-describedby="notes-help @error('notes') notes-error @enderror"
                            @error('notes') aria-invalid="true" @enderror
                        >{{ old('notes', $facility->notes) }}</textarea>
                        <div id="notes-help" class="form-text">
                            Maksimal 1000 karakter.
                        </div>
                        @error('notes')
                            <div id="notes-error" class="invalid-feedback d-block" role="alert">
                                {{ $message }}
                            </div>
                        @enderror
                    </div>

                    {{-- Tombol Aksi --}}
                    <div class="d-flex align-items-center justify-content-between pt-3 border-top">
                        <a href="{{ route('facilities.index') }}" class="btn btn-sm btn-outline-secondary">
                            &larr; Batal &amp; Kembali
                        </a>
                        <button type="submit" class="btn btn-sm btn-teal px-4">
                            Simpan Perubahan
                        </button>
                    </div>
                </form>
            </div>
        </div>
    </div>
</div>

<script>
    function toggleLocationInputs() {
        const isRoom = document.getElementById('location_type_room').checked;
        const roomContainer = document.getElementById('container_room_id');
        const areaContainer = document.getElementById('container_area_name');

        if (isRoom) {
            roomContainer.classList.remove('d-none');
            areaContainer.classList.add('d-none');
        } else {
            roomContainer.classList.add('d-none');
            areaContainer.classList.remove('d-none');
        }
    }

    document.addEventListener('DOMContentLoaded', function() {
        toggleLocationInputs();
    });
</script>
@endsection
